...
$this -> Send_Balanaces($phones, $messsage);
	...
	public function Send_Balanaces($phones, $messsage) {
		...
		$footer = "+%0A+*+DVI+-+SMT*";
		$body = $title.$messsage.$footer;
		$X = urldecode($title.$messsage.$footer.$phones) . '</br>';
		echo "http://192.168.6.19:13000/cgi-bin/sendsms?username=clinton&password=ch41sms&to=$phones&text=$body";
		exit;
		$z = file_get_contents("http://192.168.6.19:13000/cgi-bin/sendsms?username=clinton&password=ch41sms&to=$phones&text=$body");
